import { Button } from "@/components/ui/button";

const FeatureImageOne = () => {
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-14 py-28 bg-white">
      {/* Responsive Grid Container */}
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        {/* Image: order-1 on mobile, order-2 on large screens */}
        <div className="w-full order-1 lg:order-2">
          <img src="https://i.imgur.com/IsqWKo2.jpeg" alt="App preview" className="w-full h-auto object-contain" />
        </div>
        {/* Content: order-2 on mobile, order-1 on large screens */}
        <div className="flex flex-col items-start gap-6 lg:gap-12 w-full order-2 lg:order-1">
          {/* Section Title */}
          <div className="flex flex-col items-start gap-4 w-full">
            {/* Heading + Text */}
            <div className="flex flex-col items-start gap-2 w-full">
              <h2>Você escolhe quanto, quando e como deseja atender</h2>
              <p>Monte sua agenda com liberdade e conduza seus atendimentos com responsabilidade clínica e total flexibilidade. Aqui, você trabalha do seu jeito, com suporte de uma plataforma que valoriza o seu tempo e a sua escuta.</p>
            </div>
          </div>
          {/* Actions */}
          <div className="flex flex-col sm:flex-row w-full gap-4">
            <Button size="lg" className="flex w-full sm:w-[20.25rem] p-0 justify-center items-center rounded-2xl border border-white bg-secondary hover:bg-secondary/90 h-auto">
              <a
                href="https://syd-83174.bubbleapps.io/auth?t=login"
                target="_blank"
                rel="noopener noreferrer"
                className="w-full px-6 py-3.5 text-white text-xl font-semibold"
              >
                Cadastre-se agora
              </a>
            </Button>
          </div>
        </div>
      </div>
    </section>

  );
};

export default FeatureImageOne;
